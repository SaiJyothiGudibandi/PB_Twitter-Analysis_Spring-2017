...
def list(d,file):
    ...
    for i in range(0,10):
        ...
        subprocess.getoutput('hdfs dfs -copyFromLocal /home/manasa/Documents/Tweets/tweetsData-'+''.join(map(str,re.findall("'([^']*)'",str(d[i][0]))))+'.txt /Tweets/'+''.join(map(str,re.findall("'([^']*)'",str(d[i][0])))))
    subprocess.getoutput('hdfs dfs -mkdir /Tweets/Other')
    subprocess.getoutput('hdfs dfs -copyFromLocal /home/manasa/Documents/Tweets/tweetsData-Other.txt /Tweets/Other')
    subprocess.getoutput('hdfs dfs -mkdir /Tweets/None')
    subprocess.getoutput('hdfs dfs -copyFromLocal /home/manasa/Documents/Tweets/tweetsData-None.txt /Tweets/None')
